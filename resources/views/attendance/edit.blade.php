...
<div class="max-w-2xl mx-auto py-8 px-4">
    <h1 class="text-2xl font-semibold text-gray-800 mb-6">Edit Attendance</h1>
    <form method="POST" action="{{ route('attendance.update', $attendance) }}" class="bg-white shadow-lg rounded-xl p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="datetime-local" name="check_in" value="{{ old('check_in', optional($attendance->check_in)->format('Y-m-d\TH:i')) }}" class="border rounded-lg p-2 w-full focus:ring focus:ring-blue-200" />
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Check Out</label>
                <input type="datetime-local" name="check_out" value="{{ old('check_out', optional($attendance->check_out)->format('Y-m-d\TH:i')) }}" class="border rounded-lg p-2 w-full focus:ring focus:ring-blue-200" />
            </div>
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Notes</label>
            <textarea name="notes" class="border rounded-lg p-2 w-full focus:ring focus:ring-blue-200">{{ old('notes', $attendance->notes) }}</textarea>
        </div>
